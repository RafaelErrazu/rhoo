...
begin
  ...
  -- ¿Tiene turno asignado en el periodo? Sin esto no se puede saber cuales
  -- dias eran de descanso, y asumir que todos lo eran fue exactamente el bug
  -- que pagaba la quincena al doble.
  select exists (
  ...
  loop
    if r.estatus in ('completa','retardo') then
      ...
      -- Festivo trabajado: art. 75, triple (el ordinario ya va en dias
      -- trabajados, aqui van los 2 extra)
      if r.es_festivo then
        v_fest := v_fest + 1;

      -- Descanso trabajado: art. 73, salario doble ADICIONAL. Solo si hay
      -- turno asignado; si no, no hay forma de saber que era descanso.
  ...
  loop
    declare
      ...
      v_n numeric;
    begin
      -- Con turno asignado se cuentan solo los dias que caian en su turno:
      -- contar naturales inflaria las vacaciones con los descansos. Sin turno
      -- se cuentan naturales, porque filtrar por un turno inexistente daria
      -- cero dias y le borraria las vacaciones al trabajador.
  ...
  if v_hx > 0 then
    -- Art. 68: el excedente se paga al triple. Las horas no se recortan:
    -- esconder horas trabajadas es como nace una demanda laboral.
