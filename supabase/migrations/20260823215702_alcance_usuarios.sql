...
-- Cada quien puede leer su propio alcance (la UI lo necesita para saber qué
-- mostrar); solo administración puede modificarlo.
...
/* Razones sociales que este usuario alcanza. El alcance por grupo se expande a
   sus empresas, incluidas las que se creen despues. */
...
/* Atajo para políticas: ¿alcanzo esta razón social? */
...
/* El filtro se agrega a la rama de admin/RH. Las ramas de "mi propio
   expediente" y "mi equipo" NO se tocan: alguien que consulta lo suyo debe
   poder verlo aunque su perfil no tenga alcance de RH. */
...
/* Alcance de un usuario, con el catálogo para poder asignarlo. */
...
/* Reasignar un centro de empresa. Hoy solo se podia por SQL y con el orden
   correcto; aqui se hace en una transaccion con sus empleados. */
...
-- ══ 9. LA LISTA DE USUARIOS MUESTRA EL ALCANCE ═══════════════════════════
